    ...
    public partial class CartaTerminacion : System.Web.UI.Page
    {
        ...
        private void GenerarReporte(int idProgramaAlumno)
        {
            try
            {
                ...
                CartaTerminacionDataSet dataSet = new CartaTerminacionDataSet();

                // Llenar el DataSet con los datos del procedimiento almacenado
                using (SqlConnection conn = new SqlConnection(GlobalConstants.SQL))
                {
                    SqlCommand cmd = new SqlCommand("sp_Contenido_Carta_ss", conn);
                    cmd.CommandType = CommandType.StoredProcedure;
                    cmd.Parameters.AddWithValue("@kmProgramaAlumno", idProgramaAlumno);

                    SqlDataAdapter adapter = new SqlDataAdapter(cmd);
                    adapter.Fill(dataSet, "DataTable1"); // Asegúrate de que coincida con el nombre en el DataSet
                }
                // Asignar la fecha de finalización al TextBox txtFechaInicio
                if (dataSet.Tables["DataTable1"].Rows.Count > 0)
                {
                    // Obtener el valor de dFecha_fin de la primera fila
                    DataRow fila = dataSet.Tables["DataTable1"].Rows[0];
                    if (fila["dFecha_fin"] != DBNull.Value)
                    {
                        DateTime fechaFin = Convert.ToDateTime(fila["dFecha_fin"]);
                        txtFechaInicio.Text = fechaFin.ToString("yyyy-MM-dd"); // Formato para el control tipo "Date"
                    }
                }

                // Generar los códigos QR y asignarlos como binario al campo CodigoQR
                foreach (DataRow fila in dataSet.Tables["DataTable1"].Rows)
                {
                    string contenidoQR = fila["TextoQR"].ToString(); // Contenido del QR desde el campo TextoQR
                    fila["CodigoQR"] = GenerarCodigoQR(contenidoQR); // Llenar el campo con el binariohttps://localhost:44323/CartaTerminacion.aspx.cs
                }
        ...
        {
            // Usa ZXing.Net para generar el QR como un arreglo de bytes
            ...
            using (Bitmap bitmap = qrWriter.Write(texto))
